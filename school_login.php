    ...
    <div class="wrapper">
        ...
        <form class="p-3 mt-2" action="./school_login_match.php" method="POST">
            <div class="form-field d-flex align-items-center">
                ...
                <input type="email" name="email" id="email" placeholder="Email" required pattern="^\w+([\.-]?\w+)@\w+([\.-]?\w+)(\.\w{2,3})+$" title="username@gmail.com">
            </div>
            <div class="form-field d-flex align-items-center">
                <span class="fas fa-key"></span>
                <input type="password" name="password" id="password" placeholder="Password" required pattern="^(?=.*?[A-Z])(?=(.*[a-z]){1,})(?=(.*[\d]){1,})(?=(.*[\W]){1,})(?!.*\s).{8,}$" title="password must contain at least one number, one uppercase & lowercase letter, one special character and at least 8 or more characters">
                <i type="checkbox" id="icon" onclick="myFunction()" class="fa-regular fa-eye me-3"></i>
            </div>
